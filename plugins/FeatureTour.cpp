...
namespace {
    ...
    public:
        panim::AnimationInfo info() const override {
            return {"FeatureTour", 15.0, 1920, 1080, 30.0};
        }
        ...
        void on_setup(const panim::AnimationContext &ctx) override {
            ...
            fractal_ = panim::Frame(
                std::max(1, ctx.width / fractal_divisor),
                std::max(1, ctx.height / fractal_divisor));
            ...
            scenes_.add(
                "hello",
                2.6,
                [this](panim::Frame &frame, const panim::SceneTime &time) {
                    render_intro(frame, time);
                },
                0.0);
            scenes_.add(
                "mandelbulb",
                4.0,
                [this](panim::Frame &frame, const panim::SceneTime &time) {
                    render_mandelbulb(frame, time);
                });
            scenes_.add(
                "integral",
                5.2,
                [this](panim::Frame &frame, const panim::SceneTime &time) {
                    render_integral(frame, time);
                });
            scenes_.add(
                "compose",
                3.2,
                [this](panim::Frame &frame, const panim::SceneTime &time) {
                    render_outro(frame, time);
                });
        }
    ...
    private:
        ...
        void prepare_track(panim::LatexTrack &track, const char *name) {
            ...
            if (!status.ok) {
                PANIM_LOG_WARN("FeatureTour {} text unavailable: {}",
                               name,
                               status.message);
            }
        }
        ...
        void prepare_text() {
            ...
            mandelbulb_text_.add_keyframe(
                latex_text("Mandelbulb / auto-selected " + device_.device_name),
                30.0,
                0.0);

            integral_text_.set_center_norm(0.5, 0.54);
            integral_text_.set_target_height_ratio(0.135);
            integral_text_.add_keyframe(
                "I=\\int_{-\\infty}^{\\infty}e^{-x^2}\\,dx",
                0.55,
                0.0);
            integral_text_.add_keyframe(
                "I^2=\\int_{-\\infty}^{\\infty}"
            ...
            integral_caption_.add_keyframe(
                "\\text{glyph-aware equation morphing}", 30.0, 0.0);

            outro_text_.set_center_norm(0.5, 0.47);
            outro_text_.set_target_height_ratio(0.058);
            outro_text_.add_keyframe(
                "\\text{Painter + Timeline + Scenes / one small C++ plugin}",
                30.0,
                0.0);
        ...
        void draw_ambient(panim::Frame &frame,
                          panim::Color top,
                          panim::Color bottom,
                          double time_seconds) {
            ...
            for (int x = -spacing; x < ctx_.width + spacing; x += spacing) {
                painter.stroke_line(x + drift,
                                    0,
                                    x + drift - ctx_.height / 3,
                                    ctx_.height,
                                    line_width,
                                    {150, 190, 255, 255},
                                    0.08f);
        ...
        void render_intro(panim::Frame &frame, const panim::SceneTime &time) {
            draw_ambient(frame,
                         ...
                         time.local_seconds);
            panim::Painter painter(frame);
            int pulse = static_cast<int>(ctx_.height *
                                         (0.12 + 0.018 * std::sin(
                                                             time.local_seconds *
                                                             2.4)));
            painter.fill_circle(ctx_.width / 2,
                                static_cast<int>(ctx_.height * 0.48),
                                pulse * 2,
                                {74, 101, 255, 255},
                                0.09f);
        ...
        void render_mandelbulb(panim::Frame &frame,
                               const panim::SceneTime &time) {
            ...
            panim::ComputeResult result = panim::apply_compute_effect(
                fractal_, panim::ComputeEffect::Mandelbulb, params);
            if (!compute_logged_) {
                if (result.ok) {
                    PANIM_LOG_INFO("FeatureTour Mandelbulb running on {} ({})",
                                   device_.backend_name,
                                   device_.device_name);
                } else {
                    PANIM_LOG_ERROR("FeatureTour Mandelbulb failed: {}",
                                    result.message);
                }
            ...
            painter.fill_rect(0,
                              0,
                              ctx_.width,
                              static_cast<int>(ctx_.height * 0.18),
                              {4, 7, 20, 255},
                              0.58f);
        ...
        void render_integral(panim::Frame &frame,
                             const panim::SceneTime &time) {
            draw_ambient(frame,
                         {7, 25, 38, 255},
                         {12, 57, 68, 255},
                         time.local_seconds + 4.0);
            panim::Painter painter(frame);
            ...
            painter.stroke_line(graph_left,
                                baseline,
                                graph_right,
                                baseline,
                                baseline_width,
            ...
            for (int x = graph_left + 1; x <= graph_right; ++x) {
                double nx = (x - graph_left) /
                                static_cast<double>(graph_right - graph_left) *
                                6.0 -
                            3.0;
                int y = baseline - static_cast<int>(
                                       std::exp(-nx * nx) * graph_height);
                painter.stroke_line(previous_x,
                                    previous_y,
                                    x,
                                    y,
                                    curve_width,
            ...
            if (integral_text_.ready()) {
                integral_text_.render(
                    frame,
                    std::min(time.local_seconds, integral_text_.duration()));
            }
        }
        ...
        void render_outro(panim::Frame &frame, const panim::SceneTime &time) {
            ...
            painter.stroke_line(static_cast<int>(ctx_.width * 0.18),
                                static_cast<int>(ctx_.height * 0.66),
                                static_cast<int>(ctx_.width * 0.78),
                                static_cast<int>(ctx_.height * 0.66),
                                line_width,
                                accent,
                                0.24f);
        ...
        void draw_progress(panim::Frame &frame, double time_seconds) {
            double progress = scenes_.duration() > 0.0
                                  ? std::clamp(time_seconds /
                                                   scenes_.duration(),
                                               0.0,
                                               1.0)
                                  : 0.0;
