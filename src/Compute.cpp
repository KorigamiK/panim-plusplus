...
namespace panim {
    ...
    namespace {
        ...
        uint8_t to_byte(float value) { return static_cast<uint8_t>(std::clamp(value, 0.0f, 255.0f)); }
        ...
        Float3 operator-(Float3 value) { return {-value.x, -value.y, -value.z}; }

        Float3 operator*(Float3 value, float scale) { return {value.x * scale, value.y * scale, value.z * scale}; }
        ...
        Float3 mix(Float3 a, Float3 b, float amount) { return a + (b - a) * amount; }
        ...
        float mandelbulb_distance(Float3 point) {
            ...
            for (int iteration = 0; iteration < 7; ++iteration) {
                ...
                float theta = std::acos(std::clamp(z.z / safe_radius, -1.0f, 1.0f));
                ...
                z = Float3{std::sin(powered_theta) * std::cos(powered_phi), std::sin(powered_theta) * std::sin(powered_phi),
                           std::cos(powered_theta)} *
                        powered_radius +
                    point;
            }
            float safe_radius = std::max(radius, 0.00001f);
            return 0.5f * std::log(safe_radius) * safe_radius / std::max(derivative, 0.00001f);
        }
        ...
        Float3 render_mandelbulb(int x, int y, int width, int height, float time_seconds) {
            ...
            Float3 camera{2.85f * std::cos(orbit), 0.65f + 0.18f * std::sin(orbit * 0.7f), 2.85f * std::sin(orbit)};
            Float3 forward = normalize(Float3{0.0f, 0.02f, 0.0f} - camera);
            Float3 right = normalize(cross(forward, {0.0f, 1.0f, 0.0f}));
            Float3 up = cross(right, forward);
            Float3 ray_direction = normalize(forward * 1.75f + right * uv_x + up * uv_y);
            ...
            float horizon = std::clamp(0.5f + 0.5f * ray_direction.y, 0.0f, 1.0f);
            Float3 color = mix({7.0f, 10.0f, 24.0f}, {32.0f, 42.0f, 78.0f}, horizon);
            float view_glow = std::pow(std::max(dot(ray_direction, normalize(-camera)), 0.0f), 12.0f);
            ...
            if (hit) {
                ...
                float detail = 0.5f + 0.5f * std::cos(3.3f * point.y + 1.7f * point.x - time_seconds * 0.18f);
                Float3 base = mix({40.0f, 105.0f, 210.0f}, {235.0f, 92.0f, 182.0f}, detail);
                float occlusion = 1.0f - 0.42f * step_index / 52.0f;
                color = base * ((0.13f + diffuse * 1.05f) * occlusion);
                color = color + Float3{90.0f, 180.0f, 255.0f} * (rim * 0.72f);
                color = color + Float3{255.0f, 220.0f, 178.0f} * (std::pow(diffuse, 14.0f) * 0.5f);
            }
            return color;
        }

        void apply_cpu(Frame &frame, ComputeEffect effect, const ComputeParams &params) {
            ...
            for (int y = 0; y < frame.height; ++y) {
                ...
                for (int x = 0; x < frame.width; ++x) {
                    ...
                    if (effect == ComputeEffect::Invert) {
                        ...
                        float wave = 0.5f + 0.5f * std::sin((nx * 2.8f + ny * 1.7f - params.time_seconds * 0.22f) * tau);
                        float band = 0.5f + 0.5f * std::sin((nx * 0.9f - ny * 2.3f + params.time_seconds * 0.13f) * tau);
                        ...
                        Float3 color = render_mandelbulb(x, y, frame.width, frame.height, params.time_seconds);
                        target_r = color.x;
                        target_g = color.y;
                        target_b = color.z;
                    }
                    ...
                    if (effect == ComputeEffect::AnimatedGradient || effect == ComputeEffect::Mandelbulb) {
                        pixel[3] = 255;
                    }
                }
            }
        ...
        bool query_backend(ComputeBackend backend, std::string &backend_name, std::string &device_name, bool &hardware_accelerated,
                           std::string &error) {
            switch (backend) {
            ...
            {
                std::string api_name;
                if (!detail::webgpu_backend_available(device_name, api_name, hardware_accelerated, error)) {
                    return false;
                }
        ...
        bool apply_backend(ComputeBackend backend, Frame &frame, ComputeEffect effect, const ComputeParams &params, std::string &error) {
            switch (backend) {
            case ComputeBackend::WebGpu:
#ifdef PANIM_HAVE_WEBGPU
        ...
        ComputeBackend parse_backend(const char *value, bool &automatic, bool &valid) {
            automatic = true;
            valid = true;
            if (!value || !value[0])
        ...
        void initialize_runtime() {
            ...
            ComputeBackend requested = parse_backend(requested_name, automatic, valid);
            if (!valid) {
                PANIM_LOG_WARN("Unknown PANIM_COMPUTE_BACKEND '{}'; using automatic selection", requested_name);
            }
            ...
            auto select = [&](ComputeBackend backend) {
                ...
                if (!query_backend(backend, backend_name, device_name, hardware_accelerated, error)) {
                    return false;
                }
            ...
            bool selected = automatic ? select(ComputeBackend::WebGpu) : select(requested);
            if (!automatic && !selected) {
                PANIM_LOG_WARN("Requested compute backend '{}' is unavailable", compute_backend_name(requested));
            }

            if (!selected)
                select(ComputeBackend::Cpu);

            PANIM_LOG_INFO("Compute backend: {} ({})", state.info.backend_name, state.info.device_name);
        }
    ...
    ComputeResult apply_compute_effect(Frame &frame, ComputeEffect effect, const ComputeParams &params) {
        ...
        if (!state.info.hardware_accelerated && !params.allow_cpu_fallback) {
            return {false, ComputeBackend::Cpu, false, "No hardware compute backend is available"};
        }

        std::string error;
        if (apply_backend(state.info.backend, frame, effect, params, error)) {
            return {true, state.info.backend, state.info.hardware_accelerated, {}};
        }

        if (!state.failure_logged) {
            PANIM_LOG_WARN("{} compute failed: {}", state.info.backend_name, error);
            state.failure_logged = true;
        }
        if (!params.allow_cpu_fallback) {
            return {false, state.info.backend, state.info.hardware_accelerated, error};
        }

        apply_cpu(frame, effect, params);
        state.info = {ComputeBackend::Cpu, "CPU", "Portable CPU fallback after backend failure", false};
        return {true, ComputeBackend::Cpu, false, "Hardware backend failed; used CPU fallback: " + error};
    }
