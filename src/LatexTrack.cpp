...
namespace panim {
    ...
    void LatexTrack::add_keyframe(const std::string &latex, double hold_sec, double transition_sec, double height_scale) {
    ...
    Status LatexTrack::prepare(LatexRenderer &renderer, int frame_height) {
        ...
        int target_h = static_cast<int>(std::lround(frame_height * target_height_ratio_));
        std::vector<double> raster_scales;
        raster_scales.reserve(keys_.size());
        for (const auto &k : keys_) {
            if (!std::isfinite(k.height_scale) || k.height_scale <= 0.0) {
                return Status::failure("LatexTrack keyframe height scale must be positive");
            }
            int key_target_h = target_h > 0 ? std::max(1, static_cast<int>(std::lround(target_h * k.height_scale))) : -1;
            ...
            if (key_target_h > 0 && svg_dimensions(svg_path, w0, h0) && h0 > 0) {
                scale = static_cast<double>(key_target_h) / static_cast<double>(h0);
            }
            Frame f(1, 1);
        ...
        for (size_t i = 1; i < keys_.size(); ++i) {
            ...
            auto status = morph.init(keys_[i - 1].latex, keys_[i].latex, renderer, sizing);
            if (!status.ok) {
                PANIM_LOG_WARN("LatexTrack: glyph morph unavailable for transition {}: {}", i - 1, status.message);
            }
    ...
    static void alpha_over_center(const Frame &src, Frame &dst, double nx, double ny, bool tint_set, uint8_t tr, uint8_t tg, uint8_t tb) {
        ...
        for (int y = 0; y < output_height; ++y) {
            for (int x = 0; x < output_width; ++x) {
                ...
                if (ax >= 0 && ax < a.width && ay >= 0 && ay < a.height) {
                    pa = a.pixels.data() + static_cast<size_t>((ay * a.width + ax) * 4);
                }
                if (bx >= 0 && bx < b.width && by >= 0 && by < b.height) {
                    pb = b.pixels.data() + static_cast<size_t>((by * b.width + bx) * 4);
                }

                double aa = pa ? pa[3] / 255.0 : 0.0;
                double ab = pb ? pb[3] / 255.0 : 0.0;
                double am = weight_a * aa + weight_b * ab;
                double r = weight_a * (pa ? pa[0] : 0) * aa + weight_b * (pb ? pb[0] : 0) * ab;
                double g = weight_a * (pa ? pa[1] : 0) * aa + weight_b * (pb ? pb[1] : 0) * ab;
                double bl = weight_a * (pa ? pa[2] : 0) * aa + weight_b * (pb ? pb[2] : 0) * ab;
