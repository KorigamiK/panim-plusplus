...
namespace panim::detail {
    ...
    Status create_preview_surface(WGPUInstance instance,
        ...
        auto *native_window = static_cast<NSWindow *>(
            SDL_GetPointerProperty(
                properties, SDL_PROP_WINDOW_COCOA_WINDOW_POINTER, nullptr));
        if (!native_window) {
            return Status::failure(
                "SDL did not expose its Cocoa window: " +
        ...
        source.window = SDL_GetPointerProperty(
        ...
        if (wayland_display && wayland_surface) {
            ...
            source.display = SDL_GetPointerProperty(
                properties, SDL_PROP_WINDOW_X11_DISPLAY_POINTER, nullptr);
            source.window = static_cast<uint64_t>(SDL_GetNumberProperty(
...
#else
        return Status::failure(
            "Native WebGPU preview surfaces are unsupported on this platform");
#endif
