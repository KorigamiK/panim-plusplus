...
namespace panim {
    ...
    static std::string lowercase_extension(
    ...
    static bool supports_pixel_format(const AVCodec *codec,
    ...
    static Status write_packets(AVCodecContext *ctx,
                                AVFormatContext *fmt,
                                AVStream *stream) {
        ...
        while (true) {
            ...
            if (ret < 0) {
                ...
                return Status::failure(
                    "Failed to receive encoded packet: " + ffmpeg_error(ret));
            }

            av_packet_rescale_ts(pkt, ctx->time_base, stream->time_base);
            if (pkt->duration <= 0) {
                pkt->duration = av_rescale_q(
                    1, ctx->time_base, stream->time_base);
            }
            ...
            if (ret < 0) {
                av_packet_free(&pkt);
                return Status::failure(
                    "Failed to write encoded packet: " + ffmpeg_error(ret));
            }
        }
        av_packet_free(&pkt);
    ...
    VideoWriter::VideoWriter(const std::filesystem::path &path,
                             int width,
                             int height,
                             double fps,
                             int bitrate)
        : VideoWriter(path,
                      ...
                      height,
                      fps,
                      VideoWriterOptions{VideoProfile::Share, bitrate}) {}
    ...
    VideoWriter::VideoWriter(const std::filesystem::path &path,
        ...
        impl_->input_width = options.input_width > 0
                                 ? options.input_width
                                 : width;
        impl_->input_height = options.input_height > 0
        ...
        if (!codec) {
            status_ = Status::failure(
                png_output
                    ...
                    : "No suitable encoder (H.264/MPEG4) found");
            PANIM_LOG_ERROR(status_.message);
            return;
        }

        avformat_alloc_output_context2(
            &impl_->fmt, nullptr, nullptr, path.string().c_str());
        if (!impl_->fmt) {
        ...
        impl_->codec_ctx->gop_size =
            std::max(12, static_cast<int>(std::lround(fps * 2.0)));
        impl_->codec_ctx->max_b_frames = png_output ? 0 : 2;
        AVPixelFormat desired_format = AV_PIX_FMT_YUV420P;
        if (png_output) {
            desired_format = AV_PIX_FMT_RGBA;
        } else if (options.profile == VideoProfile::Master &&
                   codec->id == AV_CODEC_ID_H264) {
            desired_format = AV_PIX_FMT_YUV444P;
        }
        if (!supports_pixel_format(codec, desired_format)) {
            if (desired_format == AV_PIX_FMT_YUV444P &&
                supports_pixel_format(codec, AV_PIX_FMT_YUV420P)) {
                PANIM_LOG_WARN(
                    "Encoder {} has no 4:4:4 mode; master output will use 4:2:0",
                    codec->name);
        ...
        if (codec->id == AV_CODEC_ID_H264) {
            const char *preset = options.profile == VideoProfile::Draft
                                     ? "veryfast"
                                 : options.profile == VideoProfile::Master
                                     ? "slow"
                                     : "medium";
            av_opt_set(impl_->codec_ctx->priv_data, "preset", preset, 0);
            if (quality_mode) {
                const char *crf = options.profile == VideoProfile::Draft
                                      ? "23"
                                  : options.profile == VideoProfile::Master
                                      ? "10"
                                      : "16";
                ...
                if (options.profile == VideoProfile::Master &&
                    impl_->codec_ctx->pix_fmt == AV_PIX_FMT_YUV444P) {
                    av_opt_set(impl_->codec_ctx->priv_data,
                               "profile",
                               "high444",
                               0);
                }
        ...
        if (png_output) {
            PANIM_LOG_INFO("FFmpeg encoder: {} (lossless {}x{} PNG)",
                           codec->name,
                           width,
                           height);
        } else if (quality_mode) {
            const char *profile_name = options.profile == VideoProfile::Draft
                                           ? "draft"
                                       : options.profile ==
                                               VideoProfile::Master
                                           ? "master"
                                           : "share";
            const int crf = options.profile == VideoProfile::Draft
                                ? 23
                            : options.profile == VideoProfile::Master
                                ? 10
                                : 16;
            const char *pixel_format =
                av_get_pix_fmt_name(impl_->codec_ctx->pix_fmt);
            PANIM_LOG_INFO(
                "FFmpeg encoder: {} ({}x{}, {} fps, {} profile, CRF {}, {})",
                codec->name,
                width,
                height,
                fps,
            ...
            PANIM_LOG_INFO("FFmpeg encoder: {} ({}x{}, {} fps, {} bps)",
                           codec->name, width, height, fps, selected_bitrate);
        }

        int ret = avcodec_open2(impl_->codec_ctx, codec, nullptr);
        if (ret < 0) {
            status_ = Status::failure(
                "Failed to open codec: " + ffmpeg_error(ret));
            PANIM_LOG_ERROR(status_.message);
            return;
        }

        ret = avcodec_parameters_from_context(
            impl_->stream->codecpar, impl_->codec_ctx);
        if (ret < 0) {
        ...
        if (!(impl_->fmt->oformat->flags & AVFMT_NOFILE)) {
            ret = avio_open(
                &impl_->fmt->pb, path.string().c_str(), AVIO_FLAG_WRITE);
            if (ret < 0) {
        ...
        impl_->sws = sws_getContext(
            ...
            width,
            height,
            impl_->codec_ctx->pix_fmt,
            SWS_LANCZOS | SWS_ACCURATE_RND | SWS_FULL_CHR_H_INP |
                SWS_FULL_CHR_H_INT,
            nullptr,
            nullptr,
            nullptr);
        ...
        if (!png_output) {
            ...
            int colorspace_result = sws_setColorspaceDetails(
                impl_->sws,
                coefficients,
                1,
                coefficients,
            ...
            if (colorspace_result < 0) {
                status_ = Status::failure(
                    "Failed to configure BT.709 color conversion");
                PANIM_LOG_ERROR(status_.message);
                return;
            }
        }
    ...
    VideoWriter::~VideoWriter() {
        ...
        if (!finish_status.ok) {
            PANIM_LOG_ERROR("Failed to finalize output: {}",
    ...
    Status VideoWriter::finish() {
        ...
        if (send_result < 0 && send_result != AVERROR_EOF) {
            status_ = Status::failure(
                "Failed to flush encoder: " + ffmpeg_error(send_result));
        } else {
            Status flush_status = write_packets(
                impl_->codec_ctx, impl_->fmt, impl_->stream);
            if (!flush_status.ok)
    ...
    Status VideoWriter::write_frame(const Frame &frame) {
        ...
        if (frame.width != impl_->input_width ||
            frame.height != impl_->input_height) {
