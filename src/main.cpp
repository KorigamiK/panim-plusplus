...
#define PANIM_PLUGIN_SUFFIX ".so"
    ...
    std::string plugin_filename(std::string_view name) {
        return std::string(PANIM_PLUGIN_PREFIX) + std::string(name) +
               PANIM_PLUGIN_SUFFIX;
    }
    ...
    std::filesystem::path resolve_plugin(
            ...
            for (std::string_view name : {"FeatureTour", "Showcase", "SampleWave"}) {
                for (const auto &directory : directories) {
                    std::filesystem::path candidate =
                        directory / plugin_filename(name);
        ...
        for (const auto &directory : directories) {
            std::filesystem::path candidate =
                directory / plugin_filename(requested);
            if (path_exists(candidate))
                return candidate;
        }
        return direct;
    ...
    bool parse_size(std::string_view text, int &width, int &height) {
        ...
        if (separator == std::string_view::npos)
            return false;
        return parse_int(text.substr(0, separator), width) &&
               parse_int(text.substr(separator + 1), height);
    }
    ...
    panim::Status missing_value(std::string_view option) {
        return panim::Status::failure(
    ...
    Args parse_args(int argc, char **argv) {
        ...
        for (int index = 1; index < argc; ++index) {
            ...
            if (argument == "--plugin" || argument == "-p") {
            ...
            } else if (argument == "--start" || argument == "--from" ||
                       argument == "--time") {
                auto value = next_value();
                ...
                args.status = panim::Status::failure(
        ...
        if (!positional.empty()) {
            if (!args.plugin.empty()) {
                args.status = panim::Status::failure(
                    "Plugin was provided twice");
                return args;
            }
            args.plugin = positional[0];
        }

        auto parse_positional_double = [&](size_t position,
            ...
            if (!parse_double(positional[position], value)) {
                args.status = panim::Status::failure(
                    ...
                    std::string(positional[position]));
                return false;
            }
            target = value;
            return true;
        };
        auto parse_positional_int = [&](size_t position,
        ...
        if (!parse_positional_double(1, args.duration, "duration") ||
            !parse_positional_int(2, args.width, "width") ||
            !parse_positional_int(3, args.height, "height") ||
            !parse_positional_double(4, args.fps, "fps")) {
            return args;
        }
    ...
    void print_usage() {
        std::cout
            << "panim++ — preview and render C++ animation plugins\n\n"
            << "Usage:\n"
...
int main(int argc, char **argv) {
    ...
    if (args.command == Command::Preview) {
        if (args.start_time < 0.0 ||
            ...
            (args.fps && *args.fps <= 0.0) ||
            (args.duration && *args.duration <= 0.0) ||
            (args.frame_limit && *args.frame_limit <= 0)) {
            PANIM_LOG_ERROR("Invalid preview settings");
            return 2;
        }
        if (args.supersample != 1) {
            PANIM_LOG_ERROR(
                "--supersample applies to final render; use --size to set "
                "preview resolution");
            return 2;
        }
#ifdef PANIM_HAVE_PREVIEW
...
#else
        PANIM_LOG_ERROR(
            "Interactive preview was not built. Install SDL3 and wgpu-native, "
    ...
    double default_duration = std::max(
        1.0 / std::max(fps, 1.0), info.duration - start_time);
    ...
    std::filesystem::path output = args.output.value_or(
        std::filesystem::path("panim_out") /
        (output_stem(info.name) + (frame_command ? ".png" : ".mp4")));
    ...
    if (duration <= 0.0 || fps <= 0.0 || start_time < 0.0 ||
        width <= 0 || height <= 0 ||
        (!still_output && (width % 2 != 0 || height % 2 != 0)) ||
        (args.frame_limit && *args.frame_limit <= 0) ||
        args.supersample < 1 || args.supersample > 4) {
        PANIM_LOG_ERROR(
            "Invalid render settings: duration/fps/size must be positive, "
            "video dimensions must be even, supersample must be 1-4, and "
            "start must be non-negative");
        return 2;
    }

    long long requested_frames = still_output
                                     ? 1
                                 : args.frame_limit
                                     ? *args.frame_limit
                                     : std::llround(duration * fps);
    if (requested_frames <= 0 ||
        requested_frames > std::numeric_limits<int>::max()) {
        PANIM_LOG_ERROR("Requested frame count is out of range: {}",
                        requested_frames);
        return 2;
    }
    int frame_count = static_cast<int>(requested_frames);
    long long render_width_long =
        static_cast<long long>(width) * args.supersample;
    long long render_height_long =
        static_cast<long long>(height) * args.supersample;
    if (render_width_long > std::numeric_limits<int>::max() ||
        render_height_long > std::numeric_limits<int>::max()) {
    ...
    std::filesystem::path output_dir = output.has_parent_path()
                                           ? output.parent_path()
                                           : std::filesystem::path(".");
    std::error_code directory_error;
    std::filesystem::create_directories(output_dir, directory_error);
    if (directory_error) {
        PANIM_LOG_ERROR("Failed to create output directory {}: {}",
                        output_dir.string(),
    ...
    PANIM_LOG_INFO("Output: {} ({}x{} @ {} fps, {} frames from {} s)",
                   output.string(),
                   width,
                   height,
                   fps,
                   ...
                   start_time);
    if (args.supersample > 1) {
        PANIM_LOG_INFO("Supersampling: rendering {}x{} and downsampling to {}x{}",
                       render_width,
                       render_height,
                       width,
                       height);
    ...
    panim::VideoFrameSink sink(output,
                               width,
                               height,
                               fps,
                               writer_options);
    if (!sink.ok()) {
        PANIM_LOG_ERROR("Video writer initialization failed: {}",
                        sink.status().message);
        return 1;
    }

    panim::Status render_status = session.render_frames(sink,
                                                        start_time,
                                                        frame_count);
    if (!render_status.ok) {
    ...
    PANIM_LOG_INFO("Rendered {} to {}",
                   still_output ? "image" : "video",
                   output.string());
#else
    PANIM_LOG_ERROR(
        "FFmpeg disabled at build time; rebuild with "
